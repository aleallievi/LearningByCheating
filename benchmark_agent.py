...
def _agent_factory_hack(model_path, config, autopilot):
    ...
    if 'image_ss' in model_name:
        params = []
        for i in range(4):
            params.extend(model.location_pred[i][1].weight.data.numpy().flatten())
            params.extend(model.location_pred[i][1].bias.data.numpy().flatten())

        #with open('training/model_seed_params.txt', 'w+') as w:
        #    for p in params:
        #        w.write('%f\n' % p)
