...
def run_single(env, weather, start, target, agent_maker, seed, autopilot, show=False):
    ...
    while env.tick():
        ...
        agent_end = time.time()
        agent_times.append(agent_end - agent_st)
        ...
        if env.is_failure() or env.is_success():
            ...
            break
        env_end = time.time()
        env_times.append(env_end - env_st)
        print ('avg so far of agent time {}'.format(np.mean(agent_times)))
        #print ('avg so far of env time {}'.format(np.mean(env_times)))
    print ('num steps {}'.format(len(agent_times)))
